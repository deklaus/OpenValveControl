...
void init_system (void)
{
    ...
    init_pwm1_16bit();      // PWM to run the motor driver module
    init_uart1();
...
void init_pin_manager (void)
{
	...
	// we don't use PWM inputs, keep the defaults
...
/** Init the PWM (125 Hz).
 *  - Left aligned mode \n
 *  - Slice 1: 90% duty cycle (7.2 ms + 0.8 ms = 8 ms) \n
 *    The inactive 800 µs are available to measure the Back EMF voltage.
 *  - Slice 2: 25% duty cycle (2.0 ms + 6.0 ms = 8 ms) \n
 *    The falling edge generates the interrupt to read the motor current
 *  see datasheet ch. 30.
*/
void init_pwm1_16bit (void)
{
    PWM1CON = 0x00;     // disable PWM1
    
    PWM1ERS = 0x00;     // PWMERS External Reset Disabled (default) 
    PWM1CLK = 0x02;     // PWMCLK FOSC
    PWM1LDS = 0x00;     // PWMLDS Autoload disabled (default)
    
    // Period = (PR+1)/FOSC = 0x7CFF / 4 MHz = 8 ms
    PWM1PRL = 0xFF;     // PWMPRL 255 (Period Register LB)
    PWM1PRH = 0x7C;     // PWMPRH 124 (Period Register HB)

    PWM1CPRE = 0x03;    // PWMCPRE No clock prescale (n+1)
    PWM1PIPOS = 0x00;   // PWMPIPOS No interrupt postscale (default)
    PWM1GIR = 0x00;     // clear interrupt flags
    
    PWM1GIE = 0x00;     // Slice 'a' Parameter 1/2 match interrupt disabled
    PWM1S1CFG = 0x00;   // Slice 'a' Left aligned mode (default)  

    // Slice 1 Parameter 1 (= Active Period in Left Aligned Mode).
    // PWM1_S1P1_out is high n PWM1 clock periods at beginning of PWM1 period:
    // 0x7080 / 4 MHz = 7,2 ms
    PWM1S1P1L = 0x80;
    PWM1S1P1H = 0x70;

    // Slice 1 Parameter 2 (= Active Period in Left Aligned Mode).
    // PWM1_S1P2_out is high n PWM1 clock periods at beginning of PWM1 period:
    // 0x1F40 / 4 MHz = 2 ms (25% duty cycle)
    PWM1S1P2L = 0x40;
    PWM1S1P2H = 0x1F;
        
    PIE4bits.PWM1IE = 0;    // PWM1_16BIT parameter interrupt enable bit
    PIE4bits.PWM1PIE = 0;   // PWM1_16BIT period interrupt enable bit
    PIR4bits.PWM1PIF = 0;   // Clear PWM1_16BIT period interrupt flag
//  PIR4bits.PWM1IF = 0;    // Clear PWM1_16BIT interrupt flag (READ ONLY!)
    PWM1GIRbits.S1P1IF = 0; // Clear PWM1_16BIT slice 1, output 1 interrupt flag
    PWM1GIRbits.S1P2IF = 0; // Clear PWM1_16BIT slice 1, output 2 interrupt flag
